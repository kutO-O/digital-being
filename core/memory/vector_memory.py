...
class VectorMemory:
    ...
    def init(self) -> None:
        ...
        self._create_table()
        log.info(f"VectorMemory initialised. DB: {self._db_path}")
    ...
    def add(
        ...
        """
        Store an embedding.
        Silently skips if embedding is empty.
        Returns new row id, or None on failure.
        """
        if not embedding:
            log.debug("VectorMemory.add(): empty embedding — skipping.")
            return None
    ...
    def search(
        ...
        Returns [] if DB is empty or query_embedding is empty.
        """
    ...
    def delete_old(self, days: int = 30) -> int:
        ...
        Phase 1: no importance_score check (no join with episodic).
        """
