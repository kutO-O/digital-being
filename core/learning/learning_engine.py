...
import json
import logging
import time  # FIX: был в конце файла, теперь на верху
...
class LearningEngine:
    ...
    def learn_from_goal(self, goal: "GoalNode", tree: "GoalTree") -> None:
        ...
        existing = self._find_similar_pattern(pattern_data["goal_keywords"])

        if existing:
            existing.record_success(goal.actual_ticks)
            existing.add_example(goal.description, "completed", goal.actual_ticks)
            log.info(
            ...
            pattern.add_example(goal.description, "completed", goal.actual_ticks)
    ...
    def _extract_pattern(
        self, goal: "GoalNode", subtree: List["GoalNode"]
